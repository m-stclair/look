...
import { CHROMA_MAP_CONTROL_KEYS, CONTROL_GROUPS, DEFAULT_CONFIG, TINT_CONTROL_KEYS, TONE_MAP_CONTROL_KEYS, normalizeConfig, groupControlDefinitions, resetChromaMapConfig, resetTintConfig, resetToneMapConfig } from "./config.js";
...
function drawTintPreview(canvas, config, handleState = {}) {
  const frame = beginFrame(canvas);
  if (!frame) return;
...
function createTintControls(canvas, bindings) {
  const card = canvas.closest?.(".curve-preview-card") || canvas.parentElement;
...
function bindTintHandles(canvas, bindings) {
  let drag = null;
